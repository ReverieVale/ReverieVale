<div align="center">

<table border="0" cellspacing="0" cellpadding="0" style="border: none; border-collapse: collapse;">
  <tr style="border: none;">
    <td width="55%" align="left" valign="middle" style="border: none;">
      <h1>☾ ReverieVale</h1>
      <p><code>building quietly.</code></p>
      <br>
      <p><b>✦ ABOUT</b></p>
      <p>A quiet builder who loves elegant code, beautiful ideas, and meaningful details.</p>
      <br>
      <p><code>curious</code> · <code>creative</code> · <code>calm</code></p>
    </td>
    <td width="45%" align="center" valign="middle" style="border: none;">
      <img src="./assets/kaori.png" width="280" alt="Kaori">
    </td>
  </tr>
</table>

<br><br>

<p align="left"><b>✦ PROJECTS</b></p>

<table border="0" cellspacing="0" cellpadding="0" style="border: none; border-collapse: collapse; width: 100%;">
  <tr style="border: none;">
    <td width="33%" align="center" valign="top" style="border: none; padding-right: 8px;">
      <a href="https://github.com/ReverieVale/luna-notes">
        <img src="./assets/luna-notes.png" width="100%" alt="luna-notes">
      </a>
    </td>
    <td width="33%" align="center" valign="top" style="border: none; padding-right: 8px;">
      <a href="https://github.com/ReverieVale/inkscape">
        <img src="./assets/inkscape.png" width="100%" alt="inkscape">
      </a>
    </td>
    <td width="34%" align="center" valign="top" style="border: none;">
      <a href="https://github.com/ReverieVale/vale-archive">
        <img src="./assets/vale-archive.png" width="100%" alt="vale-archive">
      </a>
      <br><br>
      <a href="https://github.com/ReverieVale/quiet-shell">
        <img src="./assets/quiet-shell.png" width="100%" alt="quiet-shell">
      </a>
    </td>
  </tr>
</table>

<br><br>

<table border="0" cellspacing="0" cellpadding="0" style="border: none; border-collapse: collapse; width: 100%;">
  <tr style="border: none;">
    <td width="50%" align="left" valign="middle" style="border: none;">
      <p><b>✦ CURRENTLY LEARNING</b></p>
      <h2>Python · Web · Linux</h2>
      <p><sub>always learning, always building.</sub></p>
    </td>
    <td width="50%" align="center" valign="middle" style="border: none;">
      <h1 style="font-size: 28px;">君の名は。</h1>
      <p>y o u r &nbsp; n a m e .</p>
    </td>
  </tr>
</table>

<br><br>
<p><sub>// thank you for visiting</sub></p>
<p>☾</p>

</div>
